...
def main():
    ...
    for i in range(400):
        particles.append(
            Particle(random.randint(0, width), random.randint(0, height), 1, random.uniform(1, 10),
                     [math.sin(random.uniform(0, 2 * math.pi)), math.cos(random.uniform(0, 2 * math.pi))], 0,
                     (255, 255, 255)))

    sun = Particle(width / 2, height / 2, 4, 100_000, [0, 0], 1, (255, 255, 0))
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    velocity = [dx * -1, dy * -1]
                    particles.append(
                        Particle(mouse_drag_start[0], mouse_drag_start[1], 1, random.uniform(1, 10), velocity, 0,
                                 (255, 255, 255)))
                elif event.button == 2:
                    mouse_pos = pygame.mouse.get_pos()
        ...
        # heaviest_particle = max(particles, key=lambda p: p.mass)
        #
        # offset_x = width // 2 - heaviest_particle.position[0]
        # offset_y = height // 2 - heaviest_particle.position[1]
        ...
        for particle in particles:
            ...
            # particle.position[0] += offset_x
            # particle.position[1] += offset_y
